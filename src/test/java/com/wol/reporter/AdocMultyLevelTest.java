...
class AdocMultyLevelTest {
    ...
    void makeComplexMapGroups() {
        ...
        strings.stream().forEach( it -> result.set(adocMultiLevelMap.makeMultiLevelMap(it, result.get())));
        Map<String, List<Object>> resultMap = (Map<String, List<Object>>) result.get();

        StringBuilder stringBuilder = adocMultiLevelMap.prettyPrint(resultMap);
        assertEquals(
                "* source\n" +
                "* source\n" +
                "** set1\n" +
