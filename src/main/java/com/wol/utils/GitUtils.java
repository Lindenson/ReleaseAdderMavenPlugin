...
public class GitUtils {
    ...
    public static BranchInfo currentAndPreviousReleaseInfo(@NotNull TreeMap<Integer, Ref> branchNames)  {
        ...
        Optional<SortedMap<Integer, Ref>> ble = le.map(Map.Entry::getKey).map(it -> branchNames.subMap(0, it)).filter(it -> !it.isEmpty());
        Ref ref = ble.map(it -> it.get(it.lastKey())).orElse(null);
        String previous = Optional.ofNullable(ref).map(Ref::getName).map(GitUtils::refNameToBranchName).orElse(null);
    ...
    public static int releaseNameToNumber(@NotNull Pattern pattern, @NotNull String releaseName, Log logger) {
        try {
            ...
            if (matcher.matches()) {
                String sBranch = matcher.group("number");
                String nBranch = sBranch.replaceAll("\\D", "");
                return Integer.parseInt(nBranch);
            }
        }
        catch (Exception e) {
    ...
    public static TreeMap<Integer, Ref> dropBranchesLaterThenCurrentBranches(@NotNull TreeMap<Integer, Ref> source, int current) {
        ...
        if (expandedTo.isPresent()) {
            return new TreeMap<>(source.subMap(0, true, expandedTo.get(), true));
        }
        return source;
    }
    ...
    public static TreeMap<Integer, Ref> expandBranchNamesAndReorderBranches(@NotNull TreeMap<Integer, Ref> branches, Log logger)  {
        List<String> keys = branches.keySet().stream().map(String::valueOf).toList();
        ...
        return maxLength.map(it ->
                branches.entrySet().stream().map(itt ->
                {
                    ...
                    return new AbstractMap.SimpleEntry<>(Integer.valueOf(sb.toString()), itt.getValue());
